...
## Description
-->
[PoP](https://github.com/leoloso/PoP) is a component model for the back-end. This repo contains documentation for PoP's implementation in PHP.

PoP in PHP is designed as a set of PHP packages distributed through [Composer](https://getcomposer.org). The core packages are:
...
**1. A data API** (currently implemented ✅)

For fetching and posting data, in 3 flavours:

1. **GraphQL-compatible:**<br/>The shape of the response mirrors the query from the client, accessible under `/page-url/api/?datastructure=graphql&fields=...`

2. **REST-compatible:**<br/>The retrieved fields are configurable, accessible under `/page-url/api/?datastructure=rest`

3. **PoP native**<br/>A relational database structure, which retrieves object data under the object's ID (normalizing all retrieved DB data), accessible under `/page-url/?output=json` (implicit fields) or `/page-url/api/?fields=...` (explicit fields)
...
**3. Rendering on client-side** (coming soon...)

Consume the API data through JavaScript templates to render the website in the client

**4. Isomorphic rendering on server-side** (coming soon...)
...
The PoP API, used for retrieving or posting data, is accessible under `/page-url/?output=json` (implicit fields) or `/page-url/api/?fields=...` (explicit fields). It represents data the following way:
...
- [GraphQL-compatible response](https://nextapi.getpop.org/2013/01/11/markup-html-tags-and-formatting/api/?fields=title|content,comments.content|date,comments.author.name|url&datastructure=graphql)
- [REST-compatible response](https://nextapi.getpop.org/2013/01/11/markup-html-tags-and-formatting/api/?fields=title|content,comments.content|date,comments.author.name|url&datastructure=rest)
